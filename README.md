...
| **recv** | 直近1秒に Next.js が受けた件数 | 送り込み。詰まっていても上がる |
| **done** | 直近1秒に完了した件数 | さばけた量。詰むと recv より下がる |
| **wait** | 受けたがまだ終わっていない件数 | 処理待ち。cpu-burst では残りが溜まる |
| **total recv** | この負荷で受けた合計 | 完了して wait が空になると消える |
| **total done** | この負荷で完了した合計 | 同上 |
| **offer** | 直近1秒に受けたCPU仕事量（CPU秒/秒） | cap（1.0）を超えると待ち行列になる。cpu-burstは数十倍 |
| **cap** | webコンテナのCPU枠（1.0） | このデモの上限 |
| **backlog** | まだ終わっていないCPU仕事量（CPU秒） | 列に残っている仕事。draining中も残る |
| **cgroup PSI cpu / io** | コンテナ単位の待ち（同上PSI） | cpu: そのサービスのCPU上限に当たっている。io: このプロセスもI/O待ち |
...
同期ディスク書き込み負荷です。コンテナにI/O上限は付けていません（一般的なコンテナ基盤でも厳密なディスク制限をかけるケースは多くはないと想定）。
